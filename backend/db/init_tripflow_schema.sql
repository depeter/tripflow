...
-- This script creates the complete schema for Tripflow's consolidated location data
...
-- Spatial indexes
CREATE INDEX idx_locations_geom ON tripflow.locations USING GIST(geom);
CREATE INDEX idx_locations_country_geom ON tripflow.locations(country, geom);

-- Search indexes
CREATE INDEX idx_locations_name_trgm ON tripflow.locations USING gin(name gin_trgm_ops);
CREATE INDEX idx_locations_description_trgm ON tripflow.locations USING gin(description gin_trgm_ops);
...
CREATE OR REPLACE FUNCTION tripflow.find_nearby_locations(
    ...
    id BIGINT,
    name VARCHAR,
    location_type tripflow.location_type,
...
BEGIN
    ...
    SELECT
        ...
        l.name,
        l.location_type,
        ST_Distance(l.geom::geography, ST_MakePoint(lon, lat)::geography) as distance_meters,
        l.rating
    FROM tripflow.locations l
    WHERE ST_DWithin(l.geom::geography, ST_MakePoint(lon, lat)::geography, radius_meters)
...
GRANT USAGE ON SCHEMA tripflow TO tripflow;

-- Grant all privileges on tables to tripflow user
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA tripflow TO tripflow;
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA tripflow TO tripflow;

-- Grant select to read-only user if needed
-- GRANT SELECT ON ALL TABLES IN SCHEMA tripflow TO tripflow_readonly;
